...
import { motionAnimation } from '@/lib/motion';
...
export function SheetModal({
  ...
  return (
    <Modal visible={visible} transparent animationType={motionAnimation()} onRequestClose={onClose}>
      <KeyboardAvoidingView
...
export function ListRow({
  ...
  if (onPress) {
    return (
      <Pressable onPress={onPress} accessibilityRole="button" accessibilityLabel={title} style={({ pressed }) => [{ backgroundColor: pressed ? Colors.surfacePress : 'transparent' }]}>
        {content}
      </Pressable>
    );
  }
...
export function ConfirmDialog({ visible, title, sub, confirmLabel, cancelLabel, onConfirm, onCancel, loading, danger }: {
  visible: boolean;
  title: string;
  sub?: string;
  confirmLabel: string;
  ...
  return (
    <SheetModal visible={visible} onClose={onCancel} title={title}>
      {sub ? <Text style={{ color: Colors.textSecondary, fontSize: FontSize.sm, textAlign: 'center', lineHeight: 19 }}>{sub}</Text> : null}
      <Btn label={confirmLabel} variant={danger ? 'danger' : 'primary'} onPress={onConfirm} loading={loading} size="lg" />
      <Btn label={cancelLabel} variant="ghost" onPress={onCancel} disabled={loading} />
    </SheetModal>
  );
}

export function ErrorCard({ message, onRetry, requestId }: { message: string; onRetry?: () => void; requestId?: string }) {
  return (
    <Card style={{ gap: Spacing.sm }}>
      <Text style={{ color: Colors.danger, fontSize: FontSize.sm }}>{message}</Text>
      {requestId ? <Text style={{ color: Colors.textFaint, fontSize: FontSize.xs }}>Request {requestId}</Text> : null}
      {onRetry ? <Btn label="Retry" variant="ghost" size="sm" onPress={onRetry} /> : null}
    </Card>
  );
}
