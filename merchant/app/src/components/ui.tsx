    ...
    <Pressable
      ...
      {loading ? (
        ...
        <View style={styles.btnInner}>
          ...
          <Text style={{ color: fg[variant], fontSize: fs, fontFamily: fonts.body700, letterSpacing: 0.2 }}>
            {label}
          </Text>
        </View>
      )}
...
export function Chip({
  ...
  return (
    <Pressable
      ...
      <Text style={[styles.chipText, selected && { color: Colors.white, fontFamily: fonts.body600 }]}>
        {label}
        {count !== undefined ? (
          <Text style={{ color: selected ? Colors.primary : toneColor, fontWeight: '700', fontVariant: NumberStyle.fontVariant }}> {count}</Text>
        ) : null}
      </Text>
    </Pressable>
  );
...
export function Pill({ label, tone }: { label: string; tone: 'neutral' | 'danger' | 'success' | 'info' | 'warning' }) {
  ...
  return (
    <View style={{ backgroundColor: c.bg, paddingHorizontal: Spacing.sm, paddingVertical: 3, borderRadius: Radius.pill }}>
      <Text style={{ color: c.fg, fontSize: FontSize.xs, fontFamily: fonts.body700, letterSpacing: 0.3 }}>{label}</Text>
    </View>
  );
}

const STATUS_META: Record<string, { label: I18nKey; tone: 'danger' | 'warning' | 'info' | 'success' | 'neutral' }> = {
  new: { label: 'ui.status.new', tone: 'danger' },
  merchant_accepted: { label: 'ui.status.merchant_accepted', tone: 'info' },
  preparing: { label: 'ui.status.preparing', tone: 'warning' },
  ready: { label: 'ui.status.ready', tone: 'info' },
  completed: { label: 'ui.status.completed', tone: 'success' },
  cancelled: { label: 'ui.status.cancelled', tone: 'neutral' },
  refunded: { label: 'ui.status.refunded', tone: 'warning' },
  failed: { label: 'ui.status.failed', tone: 'danger' },
  disputed: { label: 'ui.status.disputed', tone: 'danger' },
};

export function StatusPill({ status }: { status: string }) {
  const meta = STATUS_META[status] ?? STATUS_META.completed;
  return <Pill label={t(meta.label).toUpperCase()} tone={meta.tone} />;
}
  ...
  return (
    <Row style={{ justifyContent: 'space-between', paddingVertical: Spacing.md }}>
      <View style={{ flex: 1, paddingRight: Spacing.lg }}>
        <Text style={{ fontSize: FontSize.md, color: Colors.text, fontFamily: fonts.body500 }}>{label}</Text>
        {sub ? <Text style={{ fontSize: FontSize.sm, color: Colors.textTertiary, marginTop: 2, lineHeight: 17 }}>{sub}</Text> : null}
      </View>
      <Switch
        value={value}
        ...
        return (
          <Pressable
            ...
            <Text
              ...
              style={[
                styles.segmentText,
                active && { color: Colors.ink, fontFamily: fonts.body700 },
                !equal && opt.label.length > 4 && { fontSize: FontSize.xs },
              ]}>
              {opt.label}
              {opt.count !== undefined ? (
                <Text style={{ color: active ? Colors.primaryDeep : Colors.textTertiary, fontVariant: NumberStyle.fontVariant }}>
                  {' '}{opt.count}
                </Text>
              ) : null}
            </Text>
...
export function Empty({ icon, title, sub }: { icon: IconName; title: string; sub?: string }) {
  return (
    <View style={styles.empty}>
      ...
      <Text style={{ color: Colors.textSecondary, fontSize: FontSize.sm, fontFamily: fonts.body600 }}>{title}</Text>
      {sub ? <Text style={{ color: Colors.textFaint, fontSize: FontSize.xs }}>{sub}</Text> : null}
    </View>
  );
}
...
export function ListRow({
  ...
  const content = (
    <View style={styles.listRow}>
      {IconComp}
      <View style={{ flex: 1 }}>
        <Text style={{ fontSize: FontSize.md, color: danger ? Colors.danger : Colors.text, fontFamily: fonts.body500 }} numberOfLines={1}>
          {title}
        </Text>
        {sub ? <Text style={{ fontSize: FontSize.sm, color: Colors.textTertiary, marginTop: 2 }} numberOfLines={1}>{sub}</Text> : null}
      </View>
      {trailing}
      {value ? <Text style={{ color: Colors.textSecondary, fontSize: FontSize.sm, fontFamily: fonts.body500, fontVariant: NumberStyle.fontVariant }}>{value}</Text> : null}
...
export function Badge({ count }: { count: number }) {
  ...
  return (
    <View style={styles.badge}>
      <Text style={{ color: Colors.white, fontSize: 10, fontFamily: fonts.body800, fontVariant: NumberStyle.fontVariant }}>
        {count > 99 ? '99+' : count}
      </Text>
    </View>
  );
...
export function Stars({ rating, size = 13, showValue }: { rating: number; size?: number; showValue?: boolean }) {
  return (
    <Row gap={2}>
      ...
      {showValue ? <Text style={{ fontSize: FontSize.xs, color: Colors.textSecondary, marginLeft: 4, fontVariant: NumberStyle.fontVariant }}>{rating.toFixed(1)}</Text> : null}
    </Row>
  );
}

export function Avatar({ name, size = 44, color }: { name: string; size?: number; color?: string }) {
  const palette = [Colors.primaryDark, Colors.info, Colors.success, Colors.warning, 'Colors.violet', Colors.primaryDeep];
  ...
  return (
    <View
      ...
      <Text style={{ color: Colors.white, fontSize: size * 0.4, fontFamily: fonts.body800 }}>
...
export function Kpi({ label, value, delta, icon, onPress }: {
  ...
  const content = (
    <Card style={styles.kpi} onPress={onPress}>
      ...
      {delta !== undefined ? (
        <Text style={{ fontSize: FontSize.xs, fontFamily: fonts.body700, color: up ? Colors.success : Colors.danger, fontVariant: NumberStyle.fontVariant }}>
          {up ? '▲' : '▼'} {Math.abs(delta)}%
        </Text>
      ) : (
...
const styles = StyleSheet.create({
  ...
  chipText: { fontSize: FontSize.sm, color: Colors.textSecondary, fontFamily: fonts.body500 },
  sectionTitle: {
  ...
  fieldHint: { fontSize: FontSize.xs, color: Colors.textFaint },
  input: {
    borderWidth: 1,
    borderColor: Colors.border,
  ...
  segmentText: { fontSize: FontSize.sm, color: Colors.textSecondary, fontFamily: fonts.body500 },
  backdrop: {
    position: 'absolute',
    top: 0,
    bottom: 0,
  ...
  kpiLabel: { fontSize: FontSize.xs, color: Colors.textTertiary, fontFamily: fonts.body500 },
  kpiValue: { fontSize: FontSize.xl, fontFamily: fonts.display700, color: Colors.text },
});