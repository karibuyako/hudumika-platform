...
test('carrier list renders and a row opens in the drawer', async ({ page }) => {
  await staffLogin(page, OWNER_PHONE)

  await page.goto('/carriers')
  const rows = page.locator('tbody tr.row-click')
  const empty = page.getByText('No carriers').first()
  ...
  await expect(drawer.getByText('API integration').first()).toBeVisible()
